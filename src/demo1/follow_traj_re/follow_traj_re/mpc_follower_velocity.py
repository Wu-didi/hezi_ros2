...
Q = np.diag([100.0, 100.0, 0.01, 0.5])  # state cost matrix
...
class MPCfollower:
    ...
    def calc_ref_trajectory(self, state, dl, pind):
        ...
        dref = np.zeros((1, T + 1))
        ncourse = len(self.cx)
        temp_ind, _ = self.calc_nearest_index(state, 0)
        ind = temp_ind + 5
    ...
    def cal_acc_and_delta(self, state):
        ...
        if self.odelta is not None:
            self.di, self.ai, self.vi = self.odelta[0], self.oa[0], ov[0]
            self.di = self.normalize_angle_rad(self.di)
            print(f"MPC Output - di: {self.di}, vi: {self.vi}")
        ...
        print("time cost in mpc :", time.time()-t0)
    ...
    def cal_dec(self):
        ...
        if (abs(np.mean(sublist)) > threshold):
            if (self.vi <= self.max_speed*3.6 / 2): return 2.0, 0 
            else: return 2.0, -1
        else: return self.vi, 0
