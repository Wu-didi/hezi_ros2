...
class RvizVisualizer(Node):
    ...
    def vs_callback(self, msg: Float32MultiArray):
        ...
        if len(self.state_queue) == 0:
            ...
            last_state = self.state_queue[-1]
            dx = self.state[0] - last_state[0]
            dy = self.state[1] - last_state[1]
    ...
    def visualize_state_queue(self):
        ...
        for i in range(len(self.state_queue)):
            pt = Point()
            x, y, _ = utm2localxy(self.state, 
                                  self.state_queue[i][0], 
                                  self.state_queue[i][1])
            pt.x = x
            pt.y = y
            pt.z = 0.0  # 假设 z 坐标为 0
    ...
    def publish_vehicle_model(self):
        ...
        self.get_logger().info(f"yaw: {yaw:.2f}, quaternion: ({qx:.3f}, {qy:.3f}, {qz:.3f}, {qw:.3f})")
